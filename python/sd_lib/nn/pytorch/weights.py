...
class WeightsRW:
    ...
    def load_rude(self, model):
        model_dict = torch.load(self._weights_fpath, map_location={'cuda:0':'cpu'})
        model.load_state_dict(model_dict, strict=False)
        return model

    def load_safe(self, model):
        snapshot_weights = torch.load(self._weights_fpath, map_location={'cuda:0':'cpu'})

        snapshot_keys = set(snapshot_weights.keys())
        model_keys = set(model.state_dict().keys())
        snapshot_extra_keys = snapshot_keys - model_keys
        model_extra_keys = model_keys - snapshot_keys
        if (len(snapshot_extra_keys) == 0) & (len(model_extra_keys) == 0):
            self._transfer_params(snapshot_weights, model)
        else:
            print('WARNING: Parameters in snapshot file and in model don\'t match!')
            model.load_state_dict(snapshot_weights, strict=False)
        return model
